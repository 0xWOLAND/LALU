...
nopHex = "00"         # later this will be "06"
...
for line in fhandle:
    ...
    print(f'[{addr:02d}] ' + hexInstr + '  ' + target2Print + (' ' *  (longestLabelLength - len(target2Print) + 2)) + origLine)
    program = program + hexInstr + ' '
    addr += 1
program = program + ((nopHex + ' ') * int(64-(len(program)/3))) # 0e is the NOP
